...
fun CoursesScreen(navController: NavHostController) {
    ...
    Scaffold(
        topBar = { ToolBar(title = LocalContext.current.getString(R.string.course)) },
        bottomBar = { BottomNavigationBar(navController) }
    ) { padding ->

        val apiResult by mainViewModel.productsScreenData.collectAsState()

        LaunchedEffect(Unit) {
            mainViewModel.getProductsScreenData()
        }
