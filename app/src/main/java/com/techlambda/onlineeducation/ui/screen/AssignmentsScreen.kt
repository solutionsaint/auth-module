...
fun AssignmentsScreen(navController: NavHostController) {
    ...
    Scaffold(
        topBar = { ToolBar(title = LocalContext.current.getString(R.string.assignment)) },
        bottomBar = { BottomNavigationBar(navController) }
    ) { padding ->

        val apiResult by mainViewModel.servicesScreenData.collectAsState()

        LaunchedEffect(Unit) {
            mainViewModel.getServicesScreenData()
        }
