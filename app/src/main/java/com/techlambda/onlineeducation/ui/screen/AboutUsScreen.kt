...
fun AboutUsScreen(navController: NavHostController) {
    ...
    Scaffold(
        topBar = { ToolBar(title = LocalContext.current.getString(R.string.about_us)) }
    ) { padding ->

        val apiResult by mainViewModel.aboutUsScreenData.collectAsState()

        LaunchedEffect(Unit) {
            mainViewModel.getAboutUsScreenData()
        }
